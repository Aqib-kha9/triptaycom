...
const ALL_LISTINGS = [
  { id: "LST-001", title: "Mountain Whisper Villa", type: "Stay", price: "₹4,500", status: "Active", views: "1.2k", bookings: 12, rating: 4.9, image: "https://images.unsplash.com/photo-1518780664697-55e3ad937233?auto=format&fit=crop&q=80&w=300" },
  { id: "LST-002", title: "River Rafting Adventure", type: "Activity", price: "₹1,200", status: "Active", views: "850", bookings: 24, rating: 4.8, image: "https://images.unsplash.com/photo-1530608139174-0675e8439b4c?auto=format&fit=crop&q=80&w=300" },
  { id: "LST-003", title: "The Creek Villa", type: "Stay", price: "₹6,200", status: "Under Review", views: "120", bookings: 0, rating: 0, image: "https://images.unsplash.com/photo-1580587771525-78b9dba3b914?auto=format&fit=crop&q=80&w=300" },
];
...
export default function VendorListingsPage() {
  ...
  const filtered = ALL_LISTINGS.filter(l => l.title.toLowerCase().includes(search.toLowerCase()));
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex-grow space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-1">
                <div>
                  <h1 className="text-xl font-bold text-zinc-900">All Listings</h1>
                  <p className="text-xs text-zinc-500 font-medium italic">Manage your entire inventory.</p>
                </div>
                <div className="flex items-center gap-3">
                  <div className="relative">
                    ...
                    <Input 
                      placeholder="Search..." 
                      className="h-10 pl-9 rounded-xl border-zinc-100 bg-white w-40 text-xs"
                      value={search}
                ...
                {filtered.map((listing) => (
                  <div key={listing.id} className="p-4 sm:p-5 flex flex-col sm:flex-row items-center justify-between gap-6 hover:bg-zinc-50/50 transition-colors group">
                    <div className="flex items-center gap-5 w-full sm:w-auto">
                      <div className="w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 group-hover:scale-105 transition-transform duration-500">
                        <img src={listing.image} alt={listing.title} className="w-full h-full object-cover" />
                      </div>
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          {listing.type === "Stay" ? <Home className="w-3 h-3 text-primary" /> : <Compass className="w-3 h-3 text-primary" />}
                          <span className={cn(
                        ...
                        <p className="text-xs font-black text-zinc-400 italic">{listing.price}<span className="text-[10px] not-italic ml-1">/unit</span></p>
                      </div>
                    </div>
          ...
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} className="relative w-full max-w-sm bg-white rounded-2xl p-8 space-y-6">
              ...
              <div className="text-center space-y-1">
                <h3 className="text-lg font-bold text-zinc-900">Delete Listing?</h3>
                <p className="text-xs text-zinc-500 font-medium">This action is permanent.</p>
              </div>
