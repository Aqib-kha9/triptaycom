...
export default function VendorMessagesPage() {
  ...
  useEffect(() => {
    ...
    newSocket.on("connect", () => {
      console.log("[Socket Vendor] Connected:", newSocket.id);
      setIsConnected(true);
      setSocketError(null);
    });

    newSocket.on("disconnect", (reason) => {
      console.log("[Socket Vendor] Disconnected:", reason);
      setIsConnected(false);
    });

    newSocket.on("connect_error", (err) => {
      console.error("[Socket Vendor] Connection error:", err.message);
  ...
  const renderEmptyConversations = () => (
    <div className="flex flex-col items-center justify-center h-full text-center p-8 space-y-3">
      ...
      <div>
        <h4 className="text-xs font-bold text-zinc-500">No Messages Yet</h4>
        <p className="text-[10px] text-zinc-400 mt-0.5 max-w-[220px]">
          Conversations with guests will appear here when they message you.
        </p>
      </div>
    </div>
  );
  ...
  const renderEmptyChat = () => (
    <div className="flex flex-col items-center justify-center h-full text-center p-8 space-y-4">
      ...
      <div>
        <h3 className="text-sm font-bold text-zinc-600">Guest Messages</h3>
        <p className="text-[11px] text-zinc-400 mt-1 max-w-[280px]">
          Select a conversation to chat with your guests. Messages are sent and
